...
полученным выписыванием цифр исходного числа в обратном порядке. Проделаем то же действие с полученной суммой и будем
повторять до тех пор, пока не получится палиндром. В гипотезе предполагается, что на каком-то шаге обязательно получится палиндром.
...
а во второй - полученный палиндром. Иначе, напечатайте "No". Если на шаге с номером, меньшим, чем N, палиндром был получен,
но получившееся после N шагов число — не палиндром, то правильным ответом является No.
...
uint32_t getNewM(uint32_t m)
{
    uint32_t reverseM = 0, copyM = m;
    while (copyM != 0)
    {
        reverseM *= 10;
        reverseM += copyM % 10;
